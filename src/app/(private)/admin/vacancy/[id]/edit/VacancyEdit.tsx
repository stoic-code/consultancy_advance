...
const EditVacancy = ({ data }: { data: any }) => {
  ...
  const {
    ...
    defaultValues: { type: '', category: '', ...data },
  })
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-xl space-y-4 px-4">
      <div>
        <CompulsoryLabel>Vacancy Title</CompulsoryLabel>
        <Input {...register('job_title')} placeholder="Eg: Software Engineer" />
        <FormErr>{errors.job_title?.message}</FormErr>
      </div>
      <div>
        <CompulsoryLabel>Location</CompulsoryLabel>
        <Input {...register('location')} placeholder="Eg: Seol, Korea" />
      ...
      <Button disabled={isPending}>Edit Vacancy</Button>
    </form>
  )
}
