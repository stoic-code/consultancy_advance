...
const Steps = () => {
  ...
  return (
    <div className="mx-auto flex w-full items-center justify-between ">
      ...
      <div
        className={`transition-width h-1 w-full flex-1 bg-muted duration-75 ease-linear  `}
      >
        <div
          className={` ${stepNo! > '1' ? 'w-full' : 'w-0'} transition-width  h-1 bg-primary duration-75 ease-linear `}
        ></div>
      </div>
      ...
      >
        <div
          className={` ${stepNo! > '2' ? 'w-full' : 'w-0'} transition-width  h-1 bg-primary duration-75 ease-linear`}
        ></div>
      </div>
      ...
      >
        <div
          className={` ${stepNo! > '3' ? 'w-full' : 'w-0'} transition-width  h-1 bg-primary duration-75 ease-linear `}
        ></div>
      </div>
      ...
      >
        <div
          className={` ${stepNo! > '4' ? 'w-full' : 'w-0'} transition-width  h-1 bg-primary duration-75 ease-linear `}
        ></div>
      </div>

      <div className="relative">
        <div
          className={`grid  ${stepNo! > '5' && 'border-primary bg-primary'}  ${isStep5 && 'border-primary bg-primary text-white'} aspect-square  w-8  place-items-center rounded-full border-2 border-muted-foreground font-bold text-muted-foreground  `}
        >
          {stepNo! > '5' ? (
            <Check size={20} className="font-bold text-white" />
          ) : (
